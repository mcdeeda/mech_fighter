...
public class unicycleMovePlayerOne : MonoBehaviour {
	...
	Rigidbody rBody;
	public float moveSpeed = 50f;  //for us to adjust the speed
	...
	void FixedUpdate () {
		if( Input.GetKeyDown (KeyCode.Comma)){  //GetKeyDown is used instead of GetKey so it relies on constant button presses instead of continuous presses
